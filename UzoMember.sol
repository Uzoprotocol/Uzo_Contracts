...
contract Uzo is IUzo, ERC721Enumerable, Ownable {
    ...
    uint256 constant MAX_MINT_PER_UZO = 2000000 ether;
    ...
    mapping(uint256 => IUzoWallet) private _wallets;
    ...
    function mint(
        ...
        for (uint256 i = 0; i < amount; i++) {
            ...
            _createUzo( tokenId, 1, _isSpecial(probability));
        }
        _checkSaleEnded();
    }
    ...
    function _createWallet(uint256 tokenId) internal returns (IUzoWallet) {
        address clone = Clones.cloneDeterministic(address(_walletLibrary), bytes32(tokenId));
        IUzoWallet(clone).init(_oracle);
        return IUzoWallet(clone);
    }
    ...
    function _createUzo(
        ...
        _mint(to, tokenId);
        IUzoWallet wallet = _createWallet(tokenId);
    ...
    function _getUnmintedTokens(uint256 tokenId) private view returns (uint256) {
        ...
        return Math.min(balance, MAX_MINT_PER_UZO - _mintedAmount[tokenId]);
    }
    ...
    function _mintPiFi(uint256 tokenId) internal {
        ...
        if (tokensToMint > 0) {
            IUzoWallet wallet = getWallet(tokenId);
    ...
    function getWallet(uint256 tokenId) public view override validToken(tokenId) returns (IUzoWallet) {
    ...
    function burn(uint256 tokenId) external override validToken(tokenId) onlyTokenOwner(tokenId) {
        require(getLevel(tokenId) > 1, "Cannot burn level 1 Uzo");
    ...
    function materialize(uint256 tokenId) external override validToken(tokenId) onlyTokenOwner(tokenId) onlyPortal {
        require(getLevel(tokenId) == 3, "Can materialize only level 3 Uzo");
    ...
    function digitalize(uint256 tokenId) external override validToken(tokenId) onlyTokenOwner(tokenId) onlyPortal {
        require(getLevel(tokenId) == 4, "Can digitalize only level 4 Uzo");
    ...
    function getInvestments(uint256 tokenId) external view returns (TokenData[] memory) {
        IUzoWallet wallet = getWallet(tokenId);
        IUzoWallet.TokenData[] memory investedTokens = wallet.listTokens();
    ...
    function getUzoData(uint256 tokenId) public view validToken(tokenId) returns (UzoData memory) {
        (uint256 pifiBalance, uint256 pifiBalanceInUSD) = getPiFiBalance(tokenId);
        UzoData memory data = UzoData(
            tokenURI(tokenId),
            tokenId,
            getLevel(tokenId),
    ...
    function UzoByOwner(
        ...
        uint256 start,
        uint256 limit
    ) external view returns (UzoData[] memory) {
        uint256 total = balanceOf(UzoOwner);
        require(start <= total, "Start index must be less than or equal to total Uzo");
        ...
        UzoData[] memory Uzo = new UzoData[](size);
        for (uint256 i = start; i < start + size; i++) {
            uint256 tokenId = tokenOfOwnerByIndex(UzoOwner, i);
            Uzo[i - start] = getUzoData(tokenId);
        }
        return Uzo;
    }
